...
const applyToEvent = asyncHandler(async (req, res) => {
    ...
    const { createdBy, volunteerId, eventId } = req.body
    ...
    const eventdocument = await Eventdocument.findById(eventId)

    // if(!donationreq){
    //     throw new ApiError(400,"donationreq not found")
    // }

    const existingEvent = await Eventdocument.findOne({
    ...
    const newEventApplication = await Eventdocument.create({
        volunteerId,
        createdBy
   ...
   // const user = await User.findById(req.user._id);
    ...
    res
    .status(200)
    .json(new ApiResponse(200,newEventApplication,"newEventApplication to event successfully"))

})

// const getalldocument = asyncHandler(async()=>{
//     const {createdBy} = req.body                     TODO: quary

//     const docs = await Eventdocument.findById()
// })
